...
TWiLight Menu++ supports custom sound files in themes. Place your sound files under the `sound` subdirectory in your theme folder, for example for the `white` theme, you would place the files at `themes/white/sound/sfx.bin` and `themes/white/sound/bgm.wav` respectively. Both files are optional, if `bgm.wav` is missing, the default music will be used. The same thing would happen with the sound effects if `sfx.bin` is missing as well.
...
You should then set the `DSi/3DS Theme Music` option in [TWiLight Menu++ Settings](faq?faq=how-do-i-access-twilight-menu-settings) to "Theme" for your custom BGM to play on the menu.
