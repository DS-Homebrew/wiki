...
#### Как исправить ситуацию, когда TWiLight Menu++ перезапускается или выдает ошибку Guru Meditation Error при запуске игры?
...
- На 3DS эта база данных доступна в приложении Universal-Updater под названием "NDS(i) Cheat Databases". Так вы сможете автоматически установить её.
...
- Папка `_nds`, расположенная в корне SD-карты, не предназначена для приложений, доступ к которым можно получить через TWiLight Menu++, поскольку она зарезервирована для системных файлов (скины, конфигурация, изображения, эмуляторы и многое другое). Если вы разместили здесь свои игры, пожалуйста, переместите их в другое место.
...
- **Темы Nintendo DSi/SEGA Saturn/Homebrew Launcher с помощью меню SELECT: ** нажмите <kbd>SELECT</kbd>, затем выберите Настройки (используйте D-PAD для перемещения по меню)
...
баннер типа banner.bin может быть анимированным и позволяет заменить иконку, которая будет отображаться в TWiLight Menu++. Они могут быть сделаны с помощью [NDS Banner Editor](https://github.com/TheGameratorT/NDS_Banner_Editor/releases), в настоящее время рекомендуется использовать [nightly версию](https://nightly.link/TheGameratorT/NDS_Banner_Editor/workflows/build/master), так как она имеет несколько улучшений по сравнению со стабильной версией.
